		...
		if(textIsDisplayable && style == STROKE){
			...
			if(strings != null && strings.length > 0){
				 for (int i = 0; i < strings.length; i++) {
					 float textWidth = paint.measureText(strings[i]);//测量字体宽度，我们需要根据字体的宽度设置在圆环中间
					if (textIsDisplayable && style == STROKE) {
						canvas.drawText(strings[i], centre - textWidth / 2,
								centre + (i+1-strings.length/2)*textSize, paint); 
					}
				}
			}
		}
